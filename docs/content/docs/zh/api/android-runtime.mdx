...
description: Android 后端启动、应用存储、Pipe 支持、本地控制和更新兼容。
---
...
Android setup 状态位于应用管理的存储目录。`updater_start_workflow` 安装或更新随包分发的后端内容，并返回终端会话元数据。Android 不依赖桌面 `uv`、系统 Python、桌面可写安装目录或 `pygit2`。
