...
打包的 C++ 可执行文件必须精确命名为 `BAAS_service`（Windows 为 `BAAS_service.exe`），通过 `--version`，并以所选 project root 在 loopback 上运行。独立持有的 remote jar 会被暂存为 `ws-scrcpy-server.jar`。这些是打包资源；当前版本的动态 BAAS runtime repository 还不是前端命令。
