...
`.github/workflows/docs-pages.yml` builds and deploys the docs. It checks out the repository, configures Pages, installs Bun, runs `bun install --frozen-lockfile` in `docs`, passes `NEXT_PUBLIC_BASE_PATH`, builds, uploads `docs/out`, and deploys it.
