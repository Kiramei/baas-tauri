...
The packaged C++ executable must be named exactly `BAAS_service` (`BAAS_service.exe` on Windows), pass `--version`, and run with the selected project root on loopback. The separately owned remote jar is staged as `ws-scrcpy-server.jar`. Desktop builds also package the native runtime-repository updater and expose only an opaque signed-plan handoff; repository URLs, refs, commits, keys, paths, and generations are not frontend-controlled inputs.
