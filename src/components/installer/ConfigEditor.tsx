...
const ConfigEditorModal = (props: ConfigEditorProps) => {
  ...
  const handleLanguageChange = (value: string) => {
    loadLocale(value).then(() => {
      let uiSettings = StorageUtil.get("uiSettings")!;
      ...
      >
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          <FormSelect
            ...
            onChange={(val: string) => {
              const newSources = [val, ...(props.config.General?.source_list?.filter((s: string) => s !== val) || [])];
              handleGeneralChange('source_list', newSources);
            }}
            options={props.config.General!.source_list!.map((e: string) => ({label: e, value: e}))}
          />
        </div>
      </motion.div>
    </div>
