...
export class InteractionHandler extends InteractionHandlerBase {
  public static SCROLL_EVENT_THROTTLING_TIME = 30; // Cap wheel forwarding at one event per 30 ms.
  ...
  protected onInteraction(event: MouseEvent | TouchEvent): void {
    ...
    if (event instanceof MouseEvent) {
      ...
      // Global listeners dispatch each touch event to every remote-view instance. Accept only
      // events whose gesture started on this canvas; adopting a gesture that began elsewhere
      // would create a remote pointer without a matching DOWN message and leave it stuck.
      if (event.target !== this.tag) {
        return;
      }
